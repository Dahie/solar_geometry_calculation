...
module SolarGeometryCalculation
  class SolarPositionCalculation
    ...
    def initialize(attributes = {})
      attributes.each { |attribute_name, attribute_value| send(:"#{attribute_name}=", attribute_value) }
      get_solar_time
    end
    ...
    def solar_elevation
      latitude_in_radian = degree_to_radian(BigDecimal(latitude.to_s))
      declination_in_radian = degree_to_radian(declination)
      hour_angle_in_radian = degree_to_radian(hour_angle)
    ...
    def solar_azimuth
      declination_in_radian = degree_to_radian(declination)
      hour_angle_in_radian = degree_to_radian(hour_angle)
      solar_elevation_in_radian = degree_to_radian(solar_elevation)
    ...
    def sunrise_sunset_base_angle
      latitude_in_radian = degree_to_radian(BigDecimal(latitude.to_s))
      declination_in_radian = degree_to_radian(declination(with_hour_and_minutes: false))
    ...
    class << self
      def format_minute(minute)
        minute.to_s.length == 1 ? "0#{minute}" : minute.to_s
      end
    end
  end
end
