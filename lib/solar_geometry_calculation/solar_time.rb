...
module SolarGeometryCalculation
  ...
  module TimeConverter
    ...
    def convert_by_offset(_time, _meridian, _longitude, _timezone_identifier)
      raise 'must be implemented by a class'
    end

    def target_time(_time, _offset_in_seconds)
      raise 'must be implemented by a class'
    end
  end
  ...
  module DaylightSavingsTimeConverter
    ...
    def convert_by_offset(time, _meridian, _longitude, timezone_identifier)
      timezone = TZInfo::Timezone.get(timezone_identifier)
      timezone_period = timezone.period_for_local(time, true)

      time = target_time(time, timezone_period.std_offset) if timezone_period.dst?
  ...
  module SolarTimeConverter
    ...
    def convert_by_offset(time, meridian, longitude, _timezone_identifier)
      minutes_difference = minutes_difference(time.year, time.month, time.day, meridian, longitude)
      target_time(time, minutes_difference * 60)
    end
